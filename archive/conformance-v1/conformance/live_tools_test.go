...
func TestLiveTunnelReverseTraffic(t *testing.T) {
	...
	port := strings.TrimSpace(remoteStdout(t, c, host, "python3 -c "+quoteShell("import socket; s=socket.socket(); s.bind(('localhost',0)); print(s.getsockname()[1]); s.close()")))
	...
	id := opened.str(t, "tunnel_id")
	t.Cleanup(func() { c.cleanup(t, "--json", "tunnel", "close", id) })
	command := "python3 -c " + quoteShell("import socket; s=socket.create_connection(('localhost',"+port+"),5); print(s.recv(100).decode()); s.close()")
...
func TestLiveTunnelPersistence(t *testing.T) {
	...
	t.Cleanup(func() {
		if !closed {
			c.cleanup(t, "--json", "tunnel", "close", id)
		}
	})
