...
func TestLiveFsTransferFailures(t *testing.T) {
	...
	c.wantErrorCode(t, "CONFIG_INVALID", "--json", "fs", "put", host,
	...
	c.wantErrorCode(t, "TRANSFER_FAILED", "--json", "fs", "put", host, good,
		"/definitely/not/here/yes.txt")

	// A directory is the other command's job, and says so before any transfer.
	c.wantErrorCode(t, "CONFIG_INVALID", "--json", "fs", "put", host, t.TempDir(), "/tmp/x")

	// `get` of a remote directory without --recursive is scp's failure to report.
	c.wantErrorCode(t, "TRANSFER_FAILED", "--json", "fs", "get", host, "/tmp",
...
func TestLiveFsSyncRejectsDangerousTarget(t *testing.T) {
	...
	for _, target := range []string{"~", "~/", "/", "/tmp"} {
		env := c.wantErrorCode(t, "SYNC_REJECTED", "--json", "fs", "sync", host, src, target, "--delete")
	...
	// remote shell.
	c.wantErrorCode(t, "SYNC_REJECTED", "--json", "fs", "sync", host, src, "/tmp/rhost-*")
