//! Session use-cases: what the caller asked for, and what the answer means.
//!
//! The session itself belongs to remote tmux, so every use-case here is one
//! helper submission; nothing is remembered between calls, and a name is resolved
//! on the remote side against the records that are actually there.
//!
