...
use super::Tunnel as TunnelData;
...
impl<'a> From<&'a TunnelData> for TunnelDto<'a> {
    fn from(value: &'a TunnelData) -> Self {
        Self {
            tunnel_id: &value.id,
            host: &value.host,
...
pub fn tunnels(rows: &[TunnelData]) -> Envelope<TunnelsDto<'_>> {
    Envelope {
        schema_version: 2,
