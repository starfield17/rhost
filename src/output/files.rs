...
use super::{Envelope, ErrorPayload, error_payload, failure};
use crate::app;
use serde::Serialize;
...
impl<'a> From<&'a app::files::Transfer> for TransferDto<'a> {
    fn from(value: &'a app::files::Transfer) -> Self {
        Self {
            source: &value.source,
...
pub fn batch<'a>(host: &str, result: &'a app::files::Batch) -> Envelope<BatchDto<'a>> {
    Envelope {
        schema_version: 2,
        operation: "fs.batch",
        ok: true,
...
pub fn file_read<'a>(host: &str, result: &'a app::files::Read) -> Envelope<FileReadDto<'a>> {
    Envelope {
        schema_version: 2,
        operation: "fs.read",
        ok: true,
