...
fn ensure_remote_parent(
    ...
    let Some(command) = super::super::backend::remote_parent_command(remote_path) else {
        return Ok(());
    };
...
pub(crate) fn run_tool(
    ...
    stop: Stop<'_>,
) -> Result<super::super::backend::Outcome, Error> {
    ...
    let outcome = super::super::backend::run(
        program,
        &args,
        timeout.unwrap_or(super::super::backend::DEFAULT_TIMEOUT),
        stop,
    );
...
pub fn put(
    ...
    super::super::backend::validate_remote_path(options.remote).map_err(validation)?;
    let local = super::super::backend::local_arg(options.local_path).map_err(validation)?;
    let destination = super::super::backend::remote_spec(options.host, options.remote);
    super::super::backend::validate_transfer_paths(&local, &destination).map_err(validation)?;
    let size = stat_local_file(Path::new(&local), "put")?;
    let timeout = options
        .timeout
        .unwrap_or(super::super::backend::DEFAULT_TIMEOUT);
    if options.parents {
    ...
    let argv = super::super::backend::scp_args(&local, &destination, &client.ssh_options())
        .map_err(validation)?;
    let outcome = run_tool(tools, &tools.scp, &argv, options.timeout, stop)?;
    Ok(Transfer {
        source: local,
        destination: super::super::backend::remote_spec(options.host, &effective),
        backend: "scp",
        size,
...
pub fn get(
    ...
    let source = super::super::backend::remote_spec(options.host, options.remote);
    let local = super::super::backend::local_arg(options.local_path).map_err(validation)?;
    super::super::backend::validate_transfer_paths(&source, &local).map_err(validation)?;
    let argv = super::super::backend::scp_args(&source, &local, &client.ssh_options())
        .map_err(validation)?;
    let outcome = run_tool(tools, &tools.scp, &argv, options.timeout, stop)?;
    ...
    if std::fs::metadata(&local).is_ok_and(|metadata| metadata.is_dir()) {
        effective = Path::new(&local)
            .join(super::super::backend::path_base(options.remote))
            .to_string_lossy()
            .to_string();
    }
...
pub(crate) fn verified(
    ...
    let (source, destination) = if download {
        (
            super::super::backend::remote_spec(host, remote_path),
            local.to_string(),
        )
    } else {
        (
    ...
    super::super::backend::validate_transfer_paths(&source, &destination).map_err(validation)?;
    ...
    let (local_file, effective_destination, remote_file) = if download {
        ...
        let file = match std::fs::metadata(named) {
            Ok(metadata) if metadata.is_dir() => {
                named.join(super::super::backend::path_base(remote_path))
            }
            _ => named.to_path_buf(),
        };
        ...
        (
            local.to_string(),
            super::super::backend::remote_spec(host, &remote_file),
            remote_file,
        )
    };
