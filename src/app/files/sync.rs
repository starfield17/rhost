...
fn summarize(
    ...
    for change in &sync.changes {
        match change.action {
            fileops::Action::Delete => sync.deletes += 1,
            fileops::Action::Create | fileops::Action::Update => sync.files += 1,
            fileops::Action::Directory => sync.directories += 1,
            fileops::Action::Skip | fileops::Action::Other => {}
        }
    }
    sync
}
...
pub fn sync(
    ...
    fileops::validate_remote_path(options.destination).map_err(validation)?;
    let local = fileops::local_arg(options.source).map_err(validation)?;
    let mut request = fileops::SyncRequest {
    ...
    let mut plan = fileops::push_args(&request, &client.ssh_options()).map_err(validation)?;
...
pub fn mirror(
    ...
    fileops::reject_sync_target(&local, options.delete).map_err(validation)?;
    if options.delete {
        ...
        match std::fs::symlink_metadata(&local) {
            Ok(_) => {
                ...
                fileops::reject_sync_target(&actual, true).map_err(validation)?;
                local = actual;
            }
    ...
    remote_has_rsync(client, options.host)?;
    let request = fileops::SyncRequest {
        host: options.host.to_string(),
    ...
    let mut plan = fileops::pull_args(&request, &client.ssh_options()).map_err(validation)?;
    if options.checksum {
        plan.args.insert(0, "--checksum".to_string());
    }
    let outcome = run_tool(tools, &tools.rsync, &plan.args, options.timeout, stop)?;
