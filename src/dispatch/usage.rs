...
use crate::audit;
use crate::cli::{FlagSpec, Help, PLAIN_FLAGS, ROOT_FLAGS_ALL, Scope, doctor_flags, exec_flags};
use crate::files as fs;
use crate::session;
use crate::tunnel;
...
fn flags(scope: Scope, leaf: &str) -> &'static [FlagSpec] {
    match scope {
        Scope::Connection | Scope::Root => PLAIN_FLAGS,
        Scope::Session => session::command::leaf_flags(leaf),
        Scope::Tunnel => tunnel::command::leaf_flags(leaf),
        Scope::Fs => fs::command::leaf_flags(leaf),
    }
}
...
pub fn leaf_or_group(scope: Scope, leaf: &str) -> Help {
...
pub(crate) fn root() -> String {
    ...
    .to_string();
    flag_lines(&mut text, ROOT_FLAGS_ALL);
    text.push_str(
...
fn group(scope: Scope) -> String {
    ...
    flag_lines(&mut text, PLAIN_FLAGS);
    text
}
...
mod tests {
    ...
    fn every_page_describes_every_flag_it_accepts() {
        for (target, accepted) in [
            (Help::Root, ROOT_FLAGS_ALL),
            (Help::Exec, exec_flags()),
            (Help::Doctor, doctor_flags()),
            (Help::Hosts, PLAIN_FLAGS),
            (Help::Version, PLAIN_FLAGS),
