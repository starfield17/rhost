...
| EXEC-003 | A remote nonzero exit is completed execution, not adapter failure; return that status locally. | `ok`, `data.execution.exit_code` | [remote nonzero test](../tests/domain.rs:remote_nonzero_is_completed_and_delivery_failure_preserves_evidence) |
| EXEC-004 | Completion requires evidence bound to this invocation and a valid exit code. | `data.execution` | [foreign completion test](../tests/domain.rs:foreign_completion_cannot_construct_verified_evidence); [VerifiedCompletion](../src/domain/identity.rs) |
...
| EXEC-006 | Timeout/cancellation with possible effects is non-retryable, including confirmed cleanup. | `error.retryable`, `data.cleanup.status` | [cleanup test](../tests/domain.rs:cleanup_is_not_permission_to_retry) |
...
| EXEC-008 | Foreground completion and local interruption are independent: retain a known exit after timeout/cancellation or inherited-pipe failure. | `data.execution`, `error.code` | [completion and interruption test](../tests/domain.rs:completion_and_interruption_are_independent); [native live race](../tests/live/exec.rs:deadline_after_completion_keeps_the_foreground_exit_code) |
| EXEC-009 | Bound JSON capture independently of live forwarding; counts describe source bytes. UTF-8 replacement must not alter counts. | `data.output.*` | [invalid UTF-8 test](../tests/domain.rs:invalid_utf8_does_not_corrupt_source_byte_evidence) |
| EXEC-010 | Output delivery failure is non-retryable; never rerun the operation or restart a partial JSON document. | `error.code` when deliverable; process status and stderr otherwise | [failed sink test](../tests/domain.rs:failed_sink_is_not_retried) |
...
| ID | Required behavior | JSON path | Executable evidence |
| --- | --- | --- | --- |
| WIRE-001 | Binary and schema versions are independent. A schema version is a compatibility generation: every output from a same-generation producer must pass that generation's published compatibility validator, or the schema version increases. v4.1.0 is the documented pre-policy v2 exception. | `schema_version`, `operation` | [DTO compatibility test](../tests/contracts.rs:actual_dtos_match_independent_schema); [maintenance policy](MAINTENANCE.md#wire-compatibility) |
...
| WIRE-004 | Remote status passes through; timeout is 124, SIGINT/SIGTERM cancellation 130/143, other adapter failure 255. These overlap possible remote statuses. | `data.execution.exit_code`, `error.code` | [domain outcome test](../tests/domain.rs:completion_and_interruption_are_independent) |
...
| WIRE-007 | Each `--help` page lists the flags its parser accepts, from the same flag specifications. Registered flags route correctly before the command; valued flag arguments are not mistaken for `--json`. JSON callers receive the corresponding usage operation and `USAGE_ERROR`, never prose on stdout. Nonzero durations smaller than one nanosecond or exceeding the supported range are refused. | `operation`, `error.code` | [help/parser black-box test](../tests/cli.rs); [routing and duration tests](../tests/cli.rs:boolean_flags_before_the_command_route_to_the_right_parser) |
| AUDIT-001 | Audit is local, bounded and fail-open; invalid lines do not hide valid entries and limited views retain only the filtered tail. Never log environment maps, file contents or send payloads. | `data.entries[]` | [audit redaction test](../tests/acceptance/audit.rs:audit_never_records_environment_values); [invalid UTF-8 test](../tests/acceptance/audit.rs:invalid_utf8_does_not_hide_valid_audit_entries_or_change_the_filtered_tail) |
...
| PERSIST-002 | v4 does not adopt or delete v3 state. Upgrade requires explicitly closing old sessions/tunnels before switching; v4 uses a separate namespace. An explicit `RHOST_REMOTE_STATE` is the complete remote v4 root used consistently by exec cleanup, doctor and sessions. | `config::v4_state_dir`, `RHOST_STATE_DIR/v4` (audit trail), `${RHOST_REMOTE_STATE:-$HOME/.local/state/rhost/v4}` | [local state isolation](../tests/persistence.rs:v4_tunnel_listing_ignores_and_preserves_legacy_state); [remote root override](../src/transport/protocol.rs:wrapper_and_cleanup_resolve_the_same_remote_state_override); [historical upgrade policy](RUST_MIGRATION.md#state-and-upgrade-policy) |
| SESSION-001 | Freeze create/list/exec/send/read/attach/recover/close. Do not introduce scheduling, windows or layout management. | `operation` | [attach refusal](../tests/acceptance/session.rs:session_attach_is_refused_under_its_own_operation); [v2 schema](../schemas/result-v2.schema.json) |
...
| SESSION-004 | All writers and close share the remote lock; paste plus Enter is one submission with invocation-local buffers. Lock contention is retryable `SESSION_UNHEALTHY`, with no submission. A send or close whose side effect is uncertain is not retryable without inspection. | `error.code`, `error.retryable` | [concurrent writer isolation](../tests/live/session.rs:killing_one_cli_keeps_the_remote_writer_and_session_alive); [helper evidence test](../src/session/remote/scripts.rs:send_and_close_only_report_success_after_their_effects_are_checked) |
| SESSION-005 | Send data is verbatim. Read cursors count raw bytes and advance by exactly the decoded page length, without omitting observation intervals. | `data.content`, `data.from`, `data.next`, `data.more` | [raw send/cursor](../tests/live/session.rs:raw_send_exit_status_unknown_target_and_recovery_are_explicit); [cursor parser test](../src/session/remote/protocol/tests.rs:a_read_needs_consistent_cursors_and_decodes_its_page) |
| SESSION-006 | Completion is invocation-bound. Missing, duplicate, malformed or contradictory evidence yields `SESSION_UNHEALTHY`, not empty success. | `data.execution`, `error.code` | [session token test](../src/session/remote/protocol/tests.rs:an_exec_result_needs_this_invocations_token_and_one_status); [VerifiedCompletion](../src/domain/identity.rs) |
| SESSION-007 | Canonical ID never becomes the caller's name. Unresolved identity is null in v2; caller reference remains separate. | `data.session_id`, `data.session_ref` | [busy session identity test](../tests/domain.rs:busy_session_has_no_exit_and_no_stdout_alias) |
...
| SESSION-010 | A create that reached the host reserves its candidate identity before submitting and reports it with `creation_status` on failure: `not_created` (explicit refusal), `unknown` (timeout/cancellation/disconnect/missing evidence), or `created` (creation evidence without a full record). A local mistake or ungeneratable id reports `data: null`. A resolved `--cwd` is recorded as an absolute path and an old record falls back to its metadata literal. | `data.session_id`, `data.session_ref`, `data.creation_status`, `data.initial_cwd` | [create uncertainty test](../src/session/ops/create.rs:silence_is_unknown_and_keeps_the_transport_cause); [failure-shape schema cases](../examples/contract_cases.rs) |
| TUNNEL-001 | Alive means the forward exists, not application health. A record is discoverable before its master can start; uncertain cleanup keeps it, and close removes a record only after the named master is confirmed stopped. | `data.tunnel_id`, `data.tunnels[]`, `error.retryable` | [tunnel persistence](../tests/live/tunnel.rs:socks_tunnel_persists_across_cli_processes_and_closes_cleanly); [uncertain cleanup test](../tests/acceptance/tunnel/open.rs:uncertain_master_cleanup_keeps_a_discoverable_record) |
...
| ID | Required behavior | JSON path | Executable evidence |
| --- | --- | --- | --- |
| SSH-001 | System OpenSSH owns resolution, authentication and host keys. Never weaken checking or install remote packages. | `error.code`, `data.capabilities` | [transport classification test](../tests/acceptance/transport.rs:transport_diagnostics_are_classified_without_fabricating_completion); [OpenSSH options builder](../src/transport/openssh.rs) |
...
| SSH-004 | `doctor` reports the binding each capability resolved to in this execution environment, under the same key set as `capabilities`, or null when absent. A failed probe reports both maps empty; doctor probes no sudo, group, full PATH, version or package ownership. | `data.capability_paths` | [capability-map unit test](../src/doctor/app.rs:capability_maps_name_the_same_keys_and_agree_on_what_was_found); [probe acceptance test](../tests/acceptance/transport.rs:a_completed_probe_reports_the_path_each_capability_resolved_to) |
...
| FS-001 | A remote path is one literal value; expand leading home shorthand remotely, reject paired outer quotes before any remote action. | `error.code`, `data.path` | [path identity test](../tests/domain.rs:identities_paths_and_hashes_are_validated) |
| FS-002 | Plain put/get use scp; get success requires a readable regular landing file. Resume/checksum use rsync and verify digest. Parent creation is explicit. | `data.backend`, `data.checksum_verified`, `error.code` | [scp invocation test](../tests/acceptance/files.rs:a_put_builds_one_scp_invocation_and_reports_the_landing_file); [missing landing file test](../tests/acceptance/files.rs:scp_success_without_a_landing_file_is_a_transfer_failure) |
...
| FS-007 | A remote file helper whose interpreter or required runtime feature is unavailable refuses before a file edit with `REMOTE_DEPENDENCY_MISSING`; doctor reports command presence, not helper readiness. | `error.code`, `data.capabilities.python3` | [unusable interpreter acceptance](../tests/acceptance/edit.rs:an_unusable_remote_python_is_a_dependency_failure_before_any_edit) |
| RELEASE-001 | Re-run the complete tagged-source gate on Linux and macOS, then execute the exact four shipping artifacts natively and verify their checksums before publication. All six jobs use the Rust version declared by Cargo. The local installer selects the same assets and verifies SHA-256 before replacement; plugin versions mirror Cargo, the sole binary version authority. | `version` result | [release ordering test](../tests/cli.rs:release_waits_for_four_native_platforms_before_publication); [release workflow](../.github/workflows/release.yml); [release contract check](../scripts/check-release-contract.sh); [installer test](../scripts/test-install.sh) |
